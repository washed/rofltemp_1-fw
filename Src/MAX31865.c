...
static inline void
assertCS (uint32_t device_num);
static inline void
deassertCS (uint32_t device_num);
  ...
  for (uint32_t device_num = 0; device_num < MAX31865_CON_DEVICES; device_num++)
    {
	  ...
	  && (MAX31865_DEVICES_SAMPLE_READY[device_num]
	      || (HAL_GPIO_ReadPin (MAX31865_DEVICES_DR_BANK_PIN[device_num][0],
				    MAX31865_DEVICES_DR_BANK_PIN[device_num][1])
		  == GPIO_PIN_RESET)))
	{
	  ...
	  if ( MAX31865_USE_CALLENDAR_VAN_DUSEN)
	    {
	      r = ((float) MAX31865_DEVICES_RTD_DATA[device_num]
		  * (float) MAX31865_REF_RESISTOR) / MAX31865_RTD_DIVIDER;
	      MAX31865_DEVICES_TEMP[device_num] = lrintf (
		  (float) ((r
		      * (MAX31865_CVD_A
			  + r * (MAX31865_CVD_B + r * MAX31865_CVD_C )))
		      * TEMP_INT_FACTOR)) - MAX31865_KELVIN_0dC;
	    }
	  else
	    {
		  ...
		  lrintf (
		      (float) (((float) MAX31865_DEVICES_RTD_DATA[device_num]
			  / 32.0) - 256.0) * (float) TEMP_INT_FACTOR);
	    }

	  switch (device_num)
	    {
	    case 0:
	      MAX31865_DEVICES_TEMP_BUFFER[current_buffer_index] =
		  MAX31865_DEVICES_TEMP[device_num];
...
initMAX31865 ()
{
  ...
  setCfgReg_MAX31865 (
      0,
      (MAX_31865_CFG_VBIAS_ON | MAX_31865_CFG_FAULT_AUTODELAY
  ...
  do
    {
      ...
      for (uint8_t device_num = 0; device_num < MAX31865_CON_DEVICES;
	  device_num++)
	{
	  getReg_MAX31865 (device_num, MAX31865_CFG_REG_RD_ADDR,
			   &status_reg[device_num], 1);
	  fault_detect_running |= (status_reg[device_num]
	      & MAX_31865_CFG_FAULT_AUTODELAY);
	  if (!(status_reg[device_num] & MAX_31865_CFG_FAULT_AUTODELAY))
	    {
	      MAX31865_FAULT_STATUS[device_num] = getFaultStatus_MAX31865 (
		  device_num);
	    }
	}
    }
  ...
  for (uint8_t device_num = 0; device_num < MAX31865_CON_DEVICES; device_num++)
      ...
      if (MAX31865_FAULT_STATUS[device_num] == 0)
	{
	  setCfgReg_MAX31865 (
	      device_num,
	      (MAX_31865_CFG_VBIAS_ON | MAX_31865_CFG_CONVAUTO_ON
		  | MAX_31865_CFG_FAULT_NONE | MAX_31865_CFG_50HZ_ON));
	}
      else
	{
...
checkMAX31865WDG ()
{
  for (uint32_t device_num = 0; device_num < MAX31865_CON_DEVICES; device_num++)
    {
      if (MAX31865_DEVICES_TIME_SINCE_LAST_READ[device_num]
	  >= MAX31865_WDG_PERIOD)
...
getRTDData_MAX31865 (uint32_t device_num)
{
  ...
  assertCS (device_num);
  HAL_SPI_TransmitReceive ( MAX31865_SPI_INSTANCE_PT, (uint8_t*) &tx_data,
			   (uint8_t*) &rx_data, 3, 100);
  deassertCS (device_num);
  MAX31865_DEVICES_SAMPLE_READY[device_num] = 0;
  MAX31865_DEVICES_RTD_DATA[device_num] =
      (((rx_data[1] << 8) | rx_data[2]) >> 1) & 0x7FFF;
}
